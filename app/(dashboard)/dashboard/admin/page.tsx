...
export default function AdminDashboardPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            console.log("Views response:", viewsRes);
            console.log("Views data length:", viewsRes.data?.length);
        ...
        <>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {quickActions.map((a) => (
                        <Link key={a.label} href={a.href}
                              className="group flex items-center gap-3 rounded-xl border bg-card p-4 hover:shadow-md transition-all hover:-translate-y-0.5">
                            <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${a.color} shrink-0`}>
                                <a.icon className="h-4 w-4 text-white"/>
                            </div>
                            <span
                                className="text-sm font-medium group-hover:text-indigo-600 transition-colors">{a.label}</span>
                        </Link>
                    ))}
                </div>

                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <StatsCard title="Total Blogs" value={isLoading ? "—" : stats?.totalBlogs ?? 0} icon={FileText}
                               gradient="bg-gradient-to-br from-indigo-500 to-indigo-600"/>
                    <StatsCard title="Published" value={isLoading ? "—" : stats?.publishedBlogs ?? 0} icon={ArrowRight}
                               gradient="bg-gradient-to-br from-emerald-500 to-emerald-600"/>
                    <StatsCard title="Total Views" value={isLoading ? "—" : formatNumber(stats?.totalViews ?? 0)}
                               icon={Eye} gradient="bg-gradient-to-br from-sky-500 to-sky-600"/>
                    <StatsCard title="Avg SEO Score" value={isLoading ? "—" : `${stats?.avgSEOScore ?? 0}/100`}
                               icon={TrendingUp} gradient="bg-gradient-to-br from-purple-500 to-purple-600"/>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                        ...
                        <CardContent>
                            {isLoading ? (
                                ...
                                <div
                                    ...
                                    No view data yet. Publish posts to start tracking.
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader><CardTitle className="text-base font-semibold flex items-center gap-2"><Sparkles
                            className="h-4 w-4 text-indigo-500"/> AI Credits</CardTitle></CardHeader>
                        <CardContent className="space-y-4">
                            <div>
                                <div className="flex justify-between text-sm mb-2">
                                    <span className="text-muted-foreground">Used this month</span>
                                    <span
                                        className="font-semibold">{stats?.aiCreditsUsed ?? 0} / {stats?.aiCreditsLimit ?? 10}</span>
                                </div>
                            ...
                            {creditPct >= 80 && (
                                <div
                                    ...
                                    <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">Running low on
                                        credits</p>
                                    <Link href="/pricing"
                                          className="text-xs text-indigo-600 hover:underline font-medium">Upgrade plan
                                        →</Link>
                                </div>
                            )}
                            <Button asChild variant="outline" size="sm" className="w-full gap-2">
                                <Link href="/dashboard/admin/blogs/new?ai=true"><Sparkles
                                    className="h-3.5 w-3.5"/> Generate Content</Link>
                            </Button>
                        </CardContent>
                    </Card>
                </div>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            ...
                            <Button asChild variant="ghost" size="sm" className="gap-1 text-indigo-600">
                                <Link href="/dashboard/admin/blogs">View all <ArrowRight
