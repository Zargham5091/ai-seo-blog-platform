...
async function getTeamContext(userId: string): Promise<TeamContext | null> {
    try {
        ...
        const owner = await UserModel.findOne({"teamMembers.userId": userId})
            .select("name email teamMembers").lean() as {
...
export default async function DashboardLayout({children}: { children: React.ReactNode }) {
    ...
    if (teamCtx && teamCtx.allowedPages.length > 0) {
        ...
        const isAlwaysAllowed = ["/dashboard/admin", "/dashboard/admin/settings", "/dashboard/user"].some(p => pathname === p);
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-background">
            ...
            <div className="flex flex-1 flex-col overflow-hidden min-w-0">
                ...
                {/* pt-14 on mobile to clear the fixed burger button, p-6 on desktop */}
                <main className="flex-1 overflow-y-auto p-4 pt-16 md:pt-4 md:p-6">
                    {children}
                </main>
            </div>
        </div>
