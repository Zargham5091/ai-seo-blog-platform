...
export async function GET(req: NextRequest) {
    try {
        ...
        if (type === "super_admin" || type === "mrr_trend" || type === "user_growth") {
            ...
            if (type === "super_admin") {
                ...
                const planDistribution = planDist.reduce(
                    (acc: Record<string, number>, p: { _id: string; count: number }) => ({...acc, [p._id]: p.count}),
                    {}
                );
            ...
            if (type === "mrr_trend") {
                ...
                return NextResponse.json({
                    ...
                    data: trend.map((t: {
                        ...
                        mrr: number;
                        newSubs: number
                    }) => ({month: MONTHS[t._id.month - 1], mrr: Math.round(t.mrr), newSubs: t.newSubs}))
                });
            }
            ...
            if (type === "user_growth") {
                ...
                return NextResponse.json({
                    ...
                    data: growth.map((g: {
                        _id: { year: number; month: number };
                        users: number
                    }) => ({month: MONTHS[g._id.month - 1], users: g.users}))
                });
            }
        }

        // ── All other types use tenant context ────────────────────────────────────
        const tenant = await getTenantContext(session.user.id);
        const tid = tenant.tenantId; // ← always owner's id

        if (type === "dashboard") {
            const [totalBlogs, publishedBlogs, draftBlogs, owner, viewAgg] = await Promise.all([
                BlogModel.countDocuments({tenantId: tid}),
                BlogModel.countDocuments({tenantId: tid, status: "published"}),
                BlogModel.countDocuments({tenantId: tid, status: "draft"}),
                UserModel.findById(tid).lean<IUserDocument>(),
                BlogModel.aggregate([
                    {$match: {tenantId: tid}},
            ...
            return NextResponse.json({
                ...
                data: {
                    ...
                    aiCreditsUsed: tenant.isOwner ? (owner?.aiCreditsUsed ?? 0) : tenant.aiCreditsUsed,
                    aiCreditsLimit: tenant.isOwner ? (owner?.aiCreditsLimit ?? 10) : tenant.aiCreditsLimit,
                    // Let the UI know if viewing as team member
                    isTeamMember: !tenant.isOwner,
                    role: tenant.role,
                },
            });
        }
