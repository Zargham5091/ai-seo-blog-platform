...
export async function GET(req: NextRequest) {
    try {
        ...
        // All team members share the owner's media folder
        const folder = `seo-platform/${tenant.tenantId}`;

        const result = await cloudinary.api.resources({
            type: "upload", prefix: folder, max_results: 24,
            next_cursor: cursor ?? undefined, resource_type: "image",
        }) as {
            resources: {
                public_id: string;
                secure_url: string;
        ...
        const media = result.resources.map((r) => ({
            id: r.public_id, url: r.secure_url, width: r.width, height: r.height,
            size: r.bytes, createdAt: r.created_at, format: r.format,
            name: r.public_id.split("/").pop() ?? "",
        }));

        return NextResponse.json({success: true, data: {media, nextCursor: result.next_cursor ?? null}});
    } catch (error) {
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (file.size > MAX_SIZE_MB * 1024 * 1024) return NextResponse.json({
            success: false,
            error: `Max size is ${MAX_SIZE_MB}MB.`
        }, {status: 400});
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        // Ensure media belongs to this tenant
        if (!id.startsWith(`seo-platform/${tenant.tenantId}/`)) {
