...
export async function POST(req: NextRequest) {
    try {
        ...
        // Add timeout to prevent hanging (45 seconds for repurpose - needs more time)
        const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("Repurposing timeout")), 45000)
        );

        const result = await Promise.race([
            repurposeContent(content, title, formats, tone, brandName),
            timeoutPromise
        ...
        if (error instanceof Error) {
            if (error.message === "Repurposing timeout") {
                return NextResponse.json({
                    success: false,
                    error: "Repurposing taking too long, please try again"
                }, {status: 504});
            }
            if (error.message.includes("API key") || error.message.includes("OpenAI") || error.message.includes("Groq")) {
