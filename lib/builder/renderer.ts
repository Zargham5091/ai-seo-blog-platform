 ...
 *
 * COMPLETE REPLACEMENT — adds animationPreset support + full SEO meta in published HTML.
 * Drop this file in place of the existing lib/builder/renderer.ts.
 *
 * Changes vs original:
 *  1. buildIframeDocument components array now accepts optional animationPreset field
 *  2. ANIMATION_CSS + ANIMATION_JS injected into every document
 *  3. wrapWithAnimation() wraps component HTML when preset is set
 *  4. buildPublishableHTML now injects full OG/Twitter/canonical meta tags
 */
...
export function buildIframeDocument(options: {
    components: {
        ...
        animationPreset?: string;           // NEW — optional, format: "presetId|duration|delay"
    }[];
...
(function(){
  document.querySelectorAll('.builder-component').forEach(function(el){
    el.addEventListener('click',function(e){
      e.stopPropagation();
      window.parent.postMessage({type:'COMPONENT_SELECTED',instanceId:el.getAttribute('data-instance')},'*');
    });
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// buildPublishableHTML — strips builder chrome, adds full SEO meta
// ─────────────────────────────────────────────────────────────────────────────
